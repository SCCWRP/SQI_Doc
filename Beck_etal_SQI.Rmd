...
toplobs <- strs_surf('bs', mod  = 'hab_mod', mod_in = 'habglm', opt_vrs = opt_vrs)
toplohy <- strs_surf('hy', mod  = 'hab_mod', mod_in = 'habglm', opt_vrs = opt_vrs)
toploH_SubNat <- strs_surf('H_SubNat', mod  = 'hab_mod', mod_in = 'habglm', opt_vrs = opt_vrs)
...
p5 <- ggplot(toplobs, aes(x = bs)) +
  ...
  theme_bw(base_size = 12,base_family = 'serif') +
  scale_colour_manual(values = biocol$col) +
  xlab('CRAM biotic structure\n') + 
  xlim(range(toplobs$bs)) +
  theme(
    axis.title.y = element_blank(),
    legend.position = 'none'
    )

p6 <- ggplot(toplohy, aes(x = hy)) +
  geom_ribbon(aes(ymin = fit - fitse, ymax = fit + fitse), fill = 'lightblue', colour = NA, alpha = 0.6) +
  geom_rug(data = sqidat[sqidat$bio_fp == 0, ], aes(x = indexscore_cram, colour = BiologicalCondition), sides = 'b') +
  geom_rug(data = sqidat[sqidat$bio_fp == 1, ], aes(x = indexscore_cram, colour = BiologicalCondition), sides = 't') +
  geom_line(aes(y = fit)) +
...
p7 <- ggplot(toploH_SubNat, aes(x = H_SubNat)) +
  geom_ribbon(aes(ymin = fit - fitse, ymax = fit + fitse), fill = 'lightblue', colour = NA, alpha = 0.6) +
  ...
  xlab('IPI evenness of\nnatural substrate') + 
  xlim(range(toploH_SubNat$H_SubNat)) +
  theme(
    axis.title.y = element_blank(),
    legend.position = 'top'
    ) + 
...
grid.arrange(
  ...
  arrangeGrob(
    ...
    arrangeGrob(p4, p5, p6, p7, ncol = 4), 
...
```{r, results = 'asis'}
# use jtools
coef_names <- c("Constant" = "(Intercept)", "log(TN)" = "log10(0.1 + TN)", "log(TP)" = "log10(0.01 + TP)", "Conductivity" = "Cond", "CRAM buffer landscape condition" = "blc", "CRAM biotic structure" = "bs", "CRAM hydrology" = "hy", "IPI evenness of natural substrate" = "H_SubNat")
...
tab <- huxres %>% 
  ...
  hline(i= 21, part = 'body', border = brd) 
tab
