...
The Stream Quality Index offers a solution for watershed managers seeking to synthesize large amounts of physical, chemical, and biological data about stream health. Using the SQI, users can both recognize large-scale patterns in data from multiple indicators, and improve how the data are communicated to high-level, non-technical environmental managers. This need is particularly pressing in regions like southern California, where large-scale landscape alteration and competing demands for water usage require managers to prioritize limited resources and management actions. As shown by the application of the SQI to stream data from the southern California, this tool can be used to prioritize sites for restoration, protection, and other management activities on a large scale.  Conversely, the SQI can be used as a valuable communication tool to highlight areas where biological objectives are not being met, perhaps motivating additional investigations to identify specific stressors in a more rigorous framework (e.g., San Diego Creek case study).  
